...
order-service 호출을 **Resilience4j 서킷 브레이커(외) + 재시도(내)** 로 감싼다(둘 다 동기 데코레이터, `decorateWithResilience`).

- **서킷 브레이커(외)** — 실패가 임계치를 넘으면 회로를 열어(OPEN) 호출 자체를 즉시 차단(fail-fast)한다. 바깥에 있으므로 OPEN이면 `CallNotPermittedException`을 던져 **재시도조차 시작하지 않는다** → 다운스트림이 죽었을 때 재시도로 부하를 증폭시키지 않는다.
- **재시도(내)** — 일시적 실패(`UNAVAILABLE`·`RESOURCE_EXHAUSTED`)에 한해 최대 3회, **지수 백오프 + jitter**로 재시도한다. 조회(GetOrders)라 idempotent해서 안전하다. 재시도가 안쪽에 있으므로 **1 논리 호출은 CB window에 결과 1건만 적재**되고(재시도 attempt마다 실패를 중복 적재하지 않음), 재시도가 blip을 흡수해 성공하면 CB는 **성공 1건**으로 본다.
